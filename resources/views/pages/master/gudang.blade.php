...
<div class="space-y-6" x-data="{ openCreate: false, openEdit: false, editItem: {} }">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Data Gudang</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Kelola lokasi penyimpanan dan inventaris daerah.</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-navy-800/30 transition-all duration-200">
                        ...
                        <td class="px-4 py-5 text-right space-x-2">
                            ...
                            <form id="delete-form-{{ $wh->id }}" action="{{ route('gudang.destroy', $wh->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" @click="triggerDelete('delete-form-{{ $wh->id }}', 'Hapus Gudang?', 'Apakah Anda yakin ingin menghapus gudang ' + '{{ $wh->nama_gudang }}' + '?')" class="text-red-600 hover:text-red-800 transition-colors">
    ...
    <x-modal title="Tambah Gudang Baru" x-show="openCreate">
        <form action="{{ route('gudang.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Lokasi / Deskripsi</label>
                <textarea name="lokasi" rows="2" class="w-full px-4 py-2 border rounded-xl dark:bg-gray-900 dark:border-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 outline-none"></textarea>
            </div>
            <div class="mt-8 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-primary-600 text-white font-bold rounded-xl shadow-lg shadow-primary-500/30">Simpan Gudang</button>
            </div>
        </form>
    </x-modal>

    <!-- Modal Edit -->
    <x-modal title="Edit Data Gudang" x-show="openEdit">
        <form :action="'/gudang/' + editItem.id" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Nama Gudang</label>
                <input type="text" name="nama_gudang" x-model="editItem.nama_gudang" required class="w-full px-4 py-2 border rounded-xl dark:bg-gray-900 dark:border-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Lokasi / Deskripsi</label>
                <textarea name="lokasi" x-model="editItem.lokasi" rows="2" class="w-full px-4 py-2 border rounded-xl dark:bg-gray-900 dark:border-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 outline-none"></textarea>
            </div>
            <div class="mt-8 flex justify-end space-x-3">
